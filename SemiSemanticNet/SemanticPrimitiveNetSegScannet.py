...
def get_training_input_data(dataset_path, batch_size, depth=6):
  ...
  with tf.name_scope('input_data_training'):
    [octree, ghost_octree, inbox_points_bytes, ghost_inbox_points_bytes, inbox_mask_bytes,
        ghost_inbox_mask_bytes, points_flag] = points_dataset(dataset_path, batch_size, depth=depth, test=False)
    ...
    inbox_gt_part_index_1 = tf.py_func(points_label_mapping, [inbox_gt_part_index, category_label[FLAGS.category]], Tout=tf.int32) # [N_point]
    ...
    ghost_inbox_gt_part_index_1 = tf.py_func(points_label_mapping, [ghost_inbox_gt_part_index, category_label[FLAGS.category]], Tout=tf.int32) # [N_ghost_point]
  ...
  return [octree, ghost_octree, inbox_node_position, ghost_inbox_node_position, inbox_batch_index, ghost_inbox_batch_index,
      inbox_gt_part_index, inbox_gt_part_index_1, ghost_inbox_gt_part_index, ghost_inbox_gt_part_index_1,
      inter_mask, ghost_inter_mask, points_flag]
...
def get_test_input_data(dataset_path, batch_size, depth=6):
  with tf.name_scope('input_data_training'):
    ...
    point_gt_part_index_1 = tf.py_func(points_label_mapping, [point_gt_part_index, category_label[FLAGS.category]], Tout=tf.int32) # [n_point]
    point_batch_index = tf.cast(node_position[:, -1], dtype=tf.int32) # [n_point]
  return octree, node_position, point_gt_part_index, point_gt_part_index_1, point_batch_index

# node_position: [n_point, 4]
def backbone(octree, node_position, depth=6, training=True, reuse=False, nempty=False):
  node_feature = network_unet34(octree, depth, training=training, reuse=reuse, nempty=nempty) # [1, C, n_node, 1]
...
def seg_header(point_feature, n_part, n_hidden=128, n_layer=2, training=True, reuse=False):
  point_predict_logits, point_hidden_feature = predict_module_v3(
      point_feature, n_part, n_hidden, n_layer, training=training, reuse=reuse) # [n_point, n_part], [n_point, n_hidden]
...
def compute_part_consistency_loss(point_predict_part_index, point_predict_prob, ghost_point_predict_prob, point_batch_index, n_part, delete_0=False):
  with tf.name_scope('part_consistency_loss'):
    ...
    with tf.control_dependencies([tf.assert_greater(point_predict_part_component, tf.constant(0.0, dtype=tf.float32))]):
      part_consistency_loss = tf.reduce_mean(part_consistency_loss / point_predict_part_component, axis=1) # [bs]
...
def train_network(depth):
  [
      ...
      inbox_gt_part_index,              # [N_point]
      inbox_gt_part_index_1,            # [N_point]
      ghost_inbox_gt_part_index,        # [N_ghost_point]
      ghost_inbox_gt_part_index_1,      # [N_ghost_point]
      inter_mask,                       # [N_point]
      ghost_inter_mask,                 # [N_ghost_point]
      points_flag                       # [bs]
  ] = get_training_input_data(FLAGS.train_data, FLAGS.train_batch_size, depth=depth)

  # get point feature
  with tf.variable_scope('seg'):
    with tf.name_scope('self'):
      inbox_point_feature = backbone(octree, inbox_node_position, depth=depth, training=True, reuse=False, nempty=FLAGS.nempty) # [1, C, N_point, 1]
      inbox_point_predict_logits, inbox_point_predict_prob, _ = seg_header(
          inbox_point_feature, n_part, training=True, reuse=False) # [N_point, n_part], [N_point, n_part]
      inter_point_predict_logits = tf.boolean_mask(inbox_point_predict_logits, inter_mask) # [N_inter, n_part]
      inter_point_predict_prob = tf.boolean_mask(inbox_point_predict_prob, inter_mask) # [N_inter, n_part]

      inbox_point_predict_logits_1, inbox_point_predict_prob_1, _ = seg_header(
          inbox_point_feature, n_part_1, training=True, reuse=False) # [N_point, n_part_1], [N_point, n_part_1]
    ...
    with tf.name_scope('ghost'):
      ghost_inbox_point_feature = backbone(ghost_octree, ghost_inbox_node_position, depth=depth, training=True, reuse=True, nempty=FLAGS.nempty) # [1, C, N_ghost_point, 1]
      ghost_inbox_point_predict_logits, ghost_inbox_point_predict_prob, _ = seg_header(
          ghost_inbox_point_feature, n_part, training=True, reuse=True) # [N_ghost_point, n_part], [N_ghost_point, n_part]
      ghost_inter_point_predict_logits = tf.boolean_mask(ghost_inbox_point_predict_logits, ghost_inter_mask) # [N_inter, n_part]
      ghost_inter_point_predict_prob = tf.boolean_mask(ghost_inbox_point_predict_prob, ghost_inter_mask) # [N_inter, n_part]

      ghost_inbox_point_predict_logits_1, ghost_inbox_point_predict_prob_1, _ = seg_header(
          ghost_inbox_point_feature, n_part_1, training=True, reuse=True) # [N_ghost_point, n_part_1], [N_ghost_point, n_part_1]
  ...
  with tf.name_scope('seg_loss'):
    with tf.name_scope('loss'):
      with tf.name_scope('self'):
        self_su_seg_loss, self_un_seg_loss = compute_segmentation_loss(
            inbox_point_predict_logits, inbox_gt_part_index, inbox_batch_index, n_part, points_flag=points_flag, delete_0=FLAGS.delete_0) # scalar, scalar
      ...
      with tf.name_scope('ghost'):
        ghost_su_seg_loss, ghost_un_seg_loss = compute_segmentation_loss(
            ghost_inbox_point_predict_logits, ghost_inbox_gt_part_index, ghost_inbox_batch_index, n_part, points_flag=points_flag, delete_0=FLAGS.delete_0) # scalar, scalar
      ...
      with tf.name_scope('total'):
        su_seg_loss = 0.5*(self_su_seg_loss + ghost_su_seg_loss) # scalar
        un_seg_loss = 0.5*(self_un_seg_loss + ghost_un_seg_loss) # scalar
    ...
    with tf.name_scope('accuracy'):
      with tf.name_scope('self'):
        su_seg_accuracy, un_seg_accuracy, inbox_point_predict_part_index = compute_segmentation_accuracy(
            inbox_point_predict_prob, inbox_gt_part_index, inbox_batch_index, points_flag=points_flag, delete_0=FLAGS.delete_0) # scalar, scalar, [N_point], [N_point]
        inter_point_predict_part_index = tf.boolean_mask(inbox_point_predict_part_index, inter_mask) # [N_inter]

        su_seg_accuracy_1, un_seg_accuracy_1, inbox_point_predict_part_index_1 = compute_segmentation_accuracy(
            inbox_point_predict_prob_1, inbox_gt_part_index_1, inbox_batch_index, points_flag=points_flag, delete_0=FLAGS.delete_0) # scalar, scalar, [N_point], [N_point]
        inter_point_predict_part_index_1 = tf.boolean_mask(inbox_point_predict_part_index_1, inter_mask) # [N_inter]
      with tf.name_scope('ghost'):
        ghost_su_seg_accuracy, ghost_un_seg_accuracy, ghost_inbox_point_predict_part_index = compute_segmentation_accuracy(
            ghost_inbox_point_predict_prob, ghost_inbox_gt_part_index, ghost_inbox_batch_index, points_flag=points_flag, delete_0=FLAGS.delete_0) # scalar, scalar, [N_ghost_point], [N_ghost_point]
        ghost_inter_point_predict_part_index = tf.boolean_mask(ghost_inbox_point_predict_part_index, ghost_inter_mask) # [N_inter]

        ghost_su_seg_accuracy_1, ghost_un_seg_accuracy_1, ghost_inbox_point_predict_part_index_1 = compute_segmentation_accuracy(
            ghost_inbox_point_predict_prob_1, ghost_inbox_gt_part_index_1, ghost_inbox_batch_index, points_flag=points_flag, delete_0=FLAGS.delete_0) # scalar, scalar, [N_ghost_point], [N_ghost_point]
  ...
  with tf.name_scope('point_consistency_loss'):
    with tf.name_scope('loss'):
      point_consistency_loss = compute_point_consistency_loss(inter_point_predict_prob, ghost_inter_point_predict_prob, inter_batch_index) # scalar
    ...
    with tf.name_scope('accuracy'):
      point_consistency_accuracy = compute_point_consistency_accuracy(inter_point_predict_part_index, ghost_inter_point_predict_part_index, inter_batch_index) # scalar
  ...
  with tf.name_scope('hierarchy_point_consistency_loss'):
    with tf.name_scope('mapping'):
      label_mapping_01 = tf.py_func(get_label_mapping, [category_label[FLAGS.category]], Tout=tf.int32) # [n_part]
      label_mapping_01_matrix = tf.one_hot(label_mapping_01, n_part_1, dtype=tf.float32) # [n_part, n_part_1]
    with tf.name_scope('prob_merge'):
      with tf.name_scope('self'):
        inter_point_predict_prob_01 = tf.matmul(inter_point_predict_prob, label_mapping_01_matrix) # [N_inter, n_part_1]
      with tf.name_scope('ghost'):
        ghost_inter_point_predict_prob_01 = tf.matmul(ghost_inter_point_predict_prob, label_mapping_01_matrix) # [N_inter, n_part_1]
    with tf.name_scope('index_merge'):
      with tf.name_scope('self'):
        inter_point_predict_part_index_of_part = tf.one_hot(inter_point_predict_part_index, n_part, dtype=tf.float32) # [N_inter, n_part]
        inter_point_predict_part_index_01_of_part = tf.matmul(inter_point_predict_part_index_of_part, label_mapping_01_matrix) # [N_inter, n_part_1]
        inter_point_predict_part_index_01 = tf.argmax(inter_point_predict_part_index_01_of_part, axis=-1, output_type=tf.int32) # [N_inter]
      with tf.name_scope('ghost'):
        ghost_inter_point_predict_part_index_of_part = tf.one_hot(ghost_inter_point_predict_part_index, n_part, dtype=tf.float32) # [N_inter, n_part]
        ghost_inter_point_predict_part_index_01_of_part = tf.matmul(ghost_inter_point_predict_part_index_of_part, label_mapping_01_matrix) # [N_inter, n_part_1]
        ghost_inter_point_predict_part_index_01 = tf.argmax(ghost_inter_point_predict_part_index_01_of_part, axis=-1, output_type=tf.int32) # [N_inter]
    with tf.name_scope('loss'):
      with tf.name_scope('self'):
        self_hierarchy_point_consistency_loss_01 = compute_point_consistency_loss(
            inter_point_predict_prob_01, ghost_inter_point_predict_prob_1, inter_batch_index) # scalar
      with tf.name_scope('ghost'):
        ghost_hierarchy_point_consistency_loss_01 = compute_point_consistency_loss(
            ghost_inter_point_predict_prob_01, inter_point_predict_prob_1, inter_batch_index) # scalar
      with tf.name_scope('total'):
        hierarchy_point_consistency_loss_01 = 0.5*(self_hierarchy_point_consistency_loss_01 + ghost_hierarchy_point_consistency_loss_01) # scalar
    with tf.name_scope('accuracy'):
      with tf.name_scope('self'):
        hierarchy_point_consistency_accuracy_01 = compute_point_consistency_accuracy(inter_point_predict_part_index_01, inter_point_predict_part_index_1, inter_batch_index) # scalar
      with tf.name_scope('ghost'):
        ghost_hierarchy_point_consistency_accuracy_01 = compute_point_consistency_accuracy(ghost_inter_point_predict_part_index_01, ghost_inter_point_predict_part_index_1, inter_batch_index) # scalar

  # part consistency loss
  with tf.name_scope('part_consistency_loss'):
    with tf.name_scope('loss'):
      with tf.name_scope('self'):
        self_part_consistency_loss = compute_part_consistency_loss(inter_point_predict_part_index,
            inter_point_predict_prob, ghost_inter_point_predict_prob, inter_batch_index, n_part, delete_0=FLAGS.delete_0) # scalar
      ...
      with tf.name_scope('ghost'):
        ghost_part_consistency_loss = compute_part_consistency_loss(ghost_inter_point_predict_part_index,
            ghost_inter_point_predict_prob, inter_point_predict_prob, inter_batch_index, n_part, delete_0=FLAGS.delete_0) # scalar
        ghost_part_consistency_loss_1 = compute_part_consistency_loss(ghost_inter_point_predict_part_index_1,
            ghost_inter_point_predict_prob_1, inter_point_predict_prob_1, inter_batch_index, n_part_1, delete_0=FLAGS.delete_0) # scalar
      part_consistency_loss = 0.5*(self_part_consistency_loss + ghost_part_consistency_loss) # scalar
      part_consistency_loss_1 = 0.5*(self_part_consistency_loss_1 + ghost_part_consistency_loss_1) # scalar
    with tf.name_scope('accuracy'):
      part_consistency_accuracy = compute_part_consistency_accuracy(
          inter_point_predict_part_index, ghost_inter_point_predict_part_index, inter_batch_index, n_part, delete_0=FLAGS.delete_0) # scalar
  ...
  # total loss
  level_0_loss = FLAGS.seg_loss_weight * su_seg_loss +\
      FLAGS.point_consistency_weight * point_consistency_loss + \
      FLAGS.part_consistency_weight * part_consistency_loss
  ...
  level_01_loss = FLAGS.hierarchy_point_consistency_weight * hierarchy_point_consistency_loss_01

  train_loss = \
      level_0_loss * FLAGS.level_0_weight + \
      level_1_loss * FLAGS.level_1_weight + \
      level_01_loss * FLAGS.level_01_weight
  ...
  with tf.name_scope('train_summary'):
    with tf.name_scope('loss'):
      ...
      summary_level_0_loss = tf.summary.scalar('level_0_loss', level_0_loss)
      summary_level_1_loss = tf.summary.scalar('level_1_loss', level_1_loss)
      summary_level_01_loss = tf.summary.scalar('level_01_loss', level_01_loss)
      summary_su_seg_loss = tf.summary.scalar('su_seg_loss', su_seg_loss)
      summary_su_seg_loss_1 = tf.summary.scalar('su_seg_loss_1', su_seg_loss_1)
      summary_point_consistency_loss = tf.summary.scalar('point_consistency_loss', point_consistency_loss)
      summary_point_consistency_loss_1 = tf.summary.scalar('point_consistency_loss_1', point_consistency_loss_1)
      summary_hierarchy_point_consistency_loss_01 = tf.summary.scalar('hierarchy_point_consistency_loss_01', hierarchy_point_consistency_loss_01)
      summary_part_consistency_loss = tf.summary.scalar('part_consistency_loss', part_consistency_loss)
    ...
    with tf.name_scope('point'):
      with tf.name_scope('self'):
        summary_un_seg_loss = tf.summary.scalar('un_seg_loss', un_seg_loss)
        summary_un_seg_loss_1 = tf.summary.scalar('un_seg_loss_1', un_seg_loss_1)
        summary_su_seg_accuracy = tf.summary.scalar('su_seg_accuracy', su_seg_accuracy)
        summary_su_seg_accuracy_1 = tf.summary.scalar('su_seg_accuracy_1', su_seg_accuracy_1)
        summary_un_seg_accuracy = tf.summary.scalar('un_seg_accuracy', un_seg_accuracy)
        summary_un_seg_accuracy_1 = tf.summary.scalar('un_seg_accuracy_1', un_seg_accuracy_1)
      with tf.name_scope('cst'):
        summary_point_consistency_accuracy = tf.summary.scalar('point_consistency_accuracy', point_consistency_accuracy)
        summary_point_consistency_accuracy_1 = tf.summary.scalar('point_consistency_accuracy_1', point_consistency_accuracy_1)
        summary_hierarchy_point_consistency_accuracy_01 = tf.summary.scalar('hierarchy_point_consistency_accuracy_01', hierarchy_point_consistency_accuracy_01)
    with tf.name_scope('part'):
      with tf.name_scope('cst'):
        summary_part_consistency_accuracy = tf.summary.scalar('part_consistency_accuracy', part_consistency_accuracy)
    ...
    with tf.name_scope('misc'):
      ...
      summary_inbox_point_predict_logits = tf.summary.histogram('inbox_point_predict_logits', inbox_point_predict_logits)
      summary_inbox_point_predict_logits_1 = tf.summary.histogram('inbox_point_predict_logits_1', inbox_point_predict_logits_1)
      summary_inbox_point_predict_prob = tf.summary.histogram('inbox_point_predict_prob', inbox_point_predict_prob)
      summary_inbox_point_predict_prob_1 = tf.summary.histogram('inbox_point_predict_prob_1', inbox_point_predict_prob_1)
      summary_points_flag = tf.summary.scalar('points_flag', tf.reduce_mean(points_flag))
    train_merged = tf.summary.merge([
        summary_train_loss,
        summary_regularizer,
        summary_level_0_loss,
        summary_level_1_loss,
        summary_level_01_loss,
        summary_su_seg_loss,
        summary_su_seg_loss_1,
        summary_point_consistency_loss,
        summary_point_consistency_loss_1,
        summary_hierarchy_point_consistency_loss_01,
        summary_part_consistency_loss,
        summary_part_consistency_loss_1,
        summary_un_seg_loss,
        summary_un_seg_loss_1,
        summary_su_seg_accuracy,
        summary_su_seg_accuracy_1,
        summary_un_seg_accuracy,
        summary_un_seg_accuracy_1,
        summary_point_consistency_accuracy,
        summary_point_consistency_accuracy_1,
        summary_hierarchy_point_consistency_accuracy_01,
        summary_part_consistency_accuracy,
        summary_part_consistency_accuracy_1,
        summary_lr_scheme,
        summary_inbox_point_predict_logits,
        summary_inbox_point_predict_logits_1,
        summary_inbox_point_predict_prob,
        summary_inbox_point_predict_prob_1,
        summary_points_flag
    ])
...
def test_network(test_data, depth=6, visual=True):
  [
      octree,                           # string
      node_position,                    # [n_point, 4]
      point_gt_part_index,              # [n_point]
      ...
      point_batch_index,                # [n_point]
  ] = get_test_input_data(test_data, FLAGS.test_batch_size, depth=depth)


  # for segmentation
  with tf.variable_scope('seg'):
    point_feature = backbone(octree, node_position, depth=depth, training=False, reuse=True, nempty=FLAGS.nempty) # [1, C, n_point, 1]
    point_predict_logits, point_predict_prob, _ = seg_header(
        point_feature, n_part, training=False, reuse=True) # [n_point, n_part], [n_point, n_part]
    point_predict_logits_1, point_predict_prob_1, _ = seg_header(
        point_feature, n_part_1, training=False, reuse=True) # [n_point, n_part_1], [n_point, n_part_1]

  # segmentation loss
  with tf.name_scope('seg_loss'):
    with tf.name_scope('level_0'):
      seg_loss = compute_segmentation_loss(
          point_predict_logits, point_gt_part_index, point_batch_index, n_part, delete_0=FLAGS.delete_0) # scalar
      seg_accuracy, point_predict_part_index = compute_segmentation_accuracy(
          point_predict_prob, point_gt_part_index, point_batch_index, delete_0=FLAGS.delete_0) # scalar, [n_point]
    ...
    with tf.name_scope('level_01'):
      label_mapping_01 = tf.py_func(get_label_mapping, [category_label[FLAGS.category]], Tout=tf.int32) # [n_part]
      label_mapping_01_matrix = tf.one_hot(label_mapping_01, n_part_1, dtype=tf.float32) # [n_part, n_part_1]
      point_predict_part_index_of_part = tf.one_hot(point_predict_part_index, n_part, dtype=tf.float32) # [n_point, n_part]
      point_predict_part_index_01_of_part = tf.matmul(point_predict_part_index_of_part, label_mapping_01_matrix) # [n_point, n_part_1]
      point_predict_part_index_01 = tf.argmax(point_predict_part_index_01_of_part, axis=-1, output_type=tf.int32) # [n_point]
      seg_accuracy_01 = compute_point_match_accuracy(point_predict_part_index_01, point_gt_part_index, point_batch_index, delete_0=FLAGS.delete_0) # [bs]
      seg_accuracy_01 = tf.reduce_mean(seg_accuracy_01) # scalar

  # part mask
  with tf.name_scope('part_mask_accuracy'):
    with tf.name_scope('level_0'):
      part_mask_accuracy, part_predict_mask, part_gt_mask = compute_part_predict_accuracy(
          point_predict_part_index, point_gt_part_index, point_batch_index, n_part, delete_0=FLAGS.delete_0) # scalar, [bs, n_part], [bs, n_part]
    with tf.name_scope('level_1'):
      part_mask_accuracy_1, part_predict_mask_1, part_gt_mask_1 = compute_part_predict_accuracy(
          point_predict_part_index_1, point_gt_part_index_1, point_batch_index, n_part_1, delete_0=FLAGS.delete_0) # scalar, [bs, n_part], [bs, n_part]
    with tf.name_scope('level_01'):
      part_mask_accuracy_01, part_predict_mask_01, _ = compute_part_predict_accuracy(
          point_predict_part_index_01, point_gt_part_index_1, point_batch_index, n_part_1, delete_0=FLAGS.delete_0) # scalar, [bs, n_part], [bs, n_part]

  # total loss
  test_loss = FLAGS.level_0_weight*FLAGS.seg_loss_weight*seg_loss + FLAGS.level_1_weight*FLAGS.seg_loss_weight*seg_loss_1

  with tf.name_scope('test_summary'):
    average_test_loss = tf.placeholder(tf.float32)
    average_seg_loss = tf.placeholder(tf.float32)
    average_seg_accuracy = tf.placeholder(tf.float32)
    average_part_mask_accuracy = tf.placeholder(tf.float32)
    average_structure_accuracy = tf.placeholder(tf.float32)
    average_part_diff = tf.placeholder(tf.float32)
    average_miou_v1 = tf.placeholder(tf.float32)
    average_miou_v2 = tf.placeholder(tf.float32)
    average_miou_v3 = tf.placeholder(tf.float32)
    ...
    average_seg_accuracy_01 = tf.placeholder(tf.float32)
    average_part_mask_accuracy_01 = tf.placeholder(tf.float32)
    average_structure_accuracy_01 = tf.placeholder(tf.float32)
    average_part_diff_01 = tf.placeholder(tf.float32)
    average_miou_v1_01 = tf.placeholder(tf.float32)
    average_miou_v2_01 = tf.placeholder(tf.float32)
    average_miou_v3_01 = tf.placeholder(tf.float32)
    summary_test_loss = tf.summary.scalar('test_loss', average_test_loss)
    summary_seg_loss = tf.summary.scalar('seg_loss', average_seg_loss)
    summary_seg_accuracy = tf.summary.scalar('seg_accuracy', average_seg_accuracy)
    summary_part_mask_accuracy = tf.summary.scalar('part_mask_accuracy', average_part_mask_accuracy)
    summary_structure_accuracy = tf.summary.scalar('structure_accuracy', average_structure_accuracy)
    summary_part_diff = tf.summary.scalar('part_diff', average_part_diff)
    summary_miou_v1 = tf.summary.scalar('miou_v1', average_miou_v1)
    summary_miou_v2 = tf.summary.scalar('miou_v2', average_miou_v2)
    summary_miou_v3 = tf.summary.scalar('miou_v3', average_miou_v3)
    ...
    summary_seg_accuracy_01 = tf.summary.scalar('seg_accuracy_01', average_seg_accuracy_01)
    summary_part_mask_accuracy_01 = tf.summary.scalar('part_mask_accuracy_01', average_part_mask_accuracy_01)
    summary_structure_accuracy_01 = tf.summary.scalar('structure_accuracy_01', average_structure_accuracy_01)
    summary_part_diff_01 = tf.summary.scalar('part_diff_01', average_part_diff_01)
    summary_miou_v1_01 = tf.summary.scalar('miou_v1_01', average_miou_v1_01)
    summary_miou_v2_01 = tf.summary.scalar('miou_v2_01', average_miou_v2_01)
    summary_miou_v3_01 = tf.summary.scalar('miou_v3_01', average_miou_v3_01)
    test_merged = tf.summary.merge([
        summary_test_loss,
        summary_seg_loss,
        summary_seg_accuracy,
        summary_part_mask_accuracy,
        summary_structure_accuracy,
        summary_part_diff,
        summary_miou_v1,
        summary_miou_v2,
        summary_miou_v3,
        ...
        summary_miou_v2_1,
        summary_miou_v3_1,
        summary_seg_accuracy_01,
        summary_part_mask_accuracy_01,
        summary_structure_accuracy_01,
        summary_part_diff_01,
        summary_miou_v1_01,
        summary_miou_v2_01,
        summary_miou_v3_01,
    ])
  return_list = [
      ...
      average_test_loss,
      average_seg_loss,
      average_seg_accuracy,
      average_part_mask_accuracy,
      average_structure_accuracy,
      average_part_diff,
      average_miou_v1,
      average_miou_v2,
      average_miou_v3,
      ...
      average_miou_v2_1,
      average_miou_v3_1,
      average_seg_accuracy_01,
      average_part_mask_accuracy_01,
      average_structure_accuracy_01,
      average_part_diff_01,
      average_miou_v1_01,
      average_miou_v2_01,
      average_miou_v3_01,
      test_loss,
      ...
      part_predict_mask_1,
      seg_accuracy_01,
      part_mask_accuracy_01,
      point_predict_part_index_01,
      part_predict_mask_01,
  ]
  if visual:
    return_list = [
        node_position,
...
def main(argv=None):
  ...
  train_summary, solver = train_network(depth=FLAGS.depth)

  # visual test
  [
      visual_node_position,
      visual_point_gt_part_index,
      visual_point_predict_part_index,
      visual_point_predict_prob,
      visual_point_gt_part_index_1,
      visual_point_predict_part_index_1,
      visual_point_predict_prob_1
  ] = test_network(FLAGS.test_data_visual, depth=FLAGS.depth, visual=True)
  ...
  [
      ...
      point_predict_part_index_01,
      part_predict_mask_01,
  ] = test_network(FLAGS.test_data, depth=FLAGS.depth, visual=False)
  ...
  tf_saver = tf.train.Saver(var_list=allvars, max_to_keep=50)
  if ckpt:
    assert(os.path.exists(FLAGS.ckpt))
    tf_restore_saver = tf.train.Saver(var_list=allvars, max_to_keep=50)
    ...
    if FLAGS.phase == 'train':
      ...
      for i in tqdm(range(start_iters, max_iter + 1)):
        ...
        if i % FLAGS.test_every_iter == 0:
          ...
          avg_seg_loss = 0
          avg_seg_accuracy = 0
          avg_part_mask_accuracy = 0
          all_point_gt_part_index = np.empty([test_iter*n_test_point], dtype=int)
          all_point_predict_part_index = np.empty([test_iter*n_test_point], dtype=int)
          all_part_gt_mask = np.empty([test_iter, n_part], dtype=int)
          all_part_predict_mask = np.empty([test_iter, n_part], dtype=int)
          avg_seg_loss_1 = 0
          ...
          avg_seg_accuracy_01 = 0
          avg_part_mask_accuracy_01 = 0
          all_point_predict_part_index_01 = np.empty([test_iter*n_test_point], dtype=int)
          all_part_predict_mask_01 = np.empty([test_iter, n_part_1], dtype=int)
          n_point_count = 0
          ...
          for it in range(test_iter):
            [
                ...
                seg_loss_value,
                seg_accuracy_value,
                part_mask_accuracy_value,
                point_gt_part_index_value,
                point_predict_part_index_value,
                part_gt_mask_value,
                part_predict_mask_value,
                seg_loss_1_value,
                ...
                seg_accuracy_01_value,
                part_mask_accuracy_01_value,
                point_predict_part_index_01_value,
                part_predict_mask_01_value,
            ] = sess.run(
                [
                    test_loss,
                    seg_loss,
            ...
            n_shape_point = point_gt_part_index_value.size
            ...
            avg_seg_loss += seg_loss_value
            avg_seg_accuracy += seg_accuracy_value
            avg_part_mask_accuracy += part_mask_accuracy_value
            all_point_gt_part_index[n_point_count:n_point_count+n_shape_point] = point_gt_part_index_value
            all_point_predict_part_index[n_point_count:n_point_count+n_shape_point] = point_predict_part_index_value
            all_part_gt_mask[it] = part_gt_mask_value
            all_part_predict_mask[it] = part_predict_mask_value
            ...
            avg_seg_accuracy_01 += seg_accuracy_01_value
            avg_part_mask_accuracy_01 += part_mask_accuracy_01_value
            all_point_predict_part_index_01[n_point_count:n_point_count+n_shape_point] = point_predict_part_index_01_value
            all_part_predict_mask_01[it] = part_predict_mask_01_value
            all_point_shape_index[n_point_count:n_point_count+n_shape_point] = it
            n_point_count += n_shape_point
          all_point_gt_part_index = all_point_gt_part_index[:n_point_count]
          all_point_predict_part_index = all_point_predict_part_index[:n_point_count]
          ...
          all_point_predict_part_index_01 = all_point_predict_part_index_01[:n_point_count]
          all_point_shape_index = all_point_shape_index[:n_point_count]
          avg_test_loss /= test_iter
          avg_seg_loss /= test_iter
          avg_seg_accuracy /= test_iter
          avg_part_mask_accuracy /= test_iter
          miou_v1, iou_v1_value = compute_iou_v1(all_point_predict_part_index, all_point_gt_part_index, all_point_shape_index, n_part, delete_0=FLAGS.delete_0)
          miou_v2, iou_v2_value = compute_iou_v2(all_point_predict_part_index, all_point_gt_part_index, n_part, delete_0=FLAGS.delete_0)
          miou_v3, iou_v3_value = compute_iou_v3(all_point_predict_part_index, all_point_gt_part_index, all_point_shape_index, n_part, delete_0=FLAGS.delete_0)
          structure_accuracy, part_diff = compute_structure_accuracy(all_part_predict_mask, all_part_gt_mask, delete_0=FLAGS.delete_0)
          ...
          avg_seg_accuracy_01 /= test_iter
          avg_part_mask_accuracy_01 /= test_iter
          miou_v1_01, iou_v1_01_value = compute_iou_v1(all_point_predict_part_index_01, all_point_gt_part_index_1, all_point_shape_index, n_part_1, delete_0=FLAGS.delete_0)
          miou_v2_01, iou_v2_01_value = compute_iou_v2(all_point_predict_part_index_01, all_point_gt_part_index_1, n_part_1, delete_0=FLAGS.delete_0)
          miou_v3_01, iou_v3_01_value = compute_iou_v3(all_point_predict_part_index_01, all_point_gt_part_index_1, all_point_shape_index, n_part_1, delete_0=FLAGS.delete_0)
          structure_accuracy_01, part_diff_01 = compute_structure_accuracy(all_part_predict_mask_01, all_part_gt_mask_1, delete_0=FLAGS.delete_0)
          summary = sess.run(test_summary,
              feed_dict={
                  average_test_loss: avg_test_loss,
                  average_seg_loss: avg_seg_loss,
                  average_seg_accuracy: avg_seg_accuracy,
                  average_part_mask_accuracy: avg_part_mask_accuracy,
                  average_structure_accuracy: structure_accuracy,
                  average_part_diff: part_diff,
                  average_miou_v1: miou_v1,
                  average_miou_v2: miou_v2,
                  average_miou_v3: miou_v3,
                  ...
                  average_seg_accuracy_01: avg_seg_accuracy_01,
                  average_part_mask_accuracy_01: avg_part_mask_accuracy_01,
                  average_structure_accuracy_01: structure_accuracy_01,
                  average_part_diff_01: part_diff_01,
                  average_miou_v1_01: miou_v1_01,
                  average_miou_v2_01: miou_v2_01,
                  average_miou_v3_01: miou_v3_01,
              })
          summary_writer.add_summary(summary, i)
          tf_saver.save(sess, os.path.join(FLAGS.logdir, 'model/iter{:06d}.ckpt'.format(i)))

          result_string = '\nIteration {}:\nLevel 0:\n miou v1     : {:5.2f}\n miou v2     : {:5.2f}\n miou v3     : {:5.2f}\n seg accu    : {:6.4f}\n part diff   :  {:6.4f}\n struct accu :  {:6.4f}\n part accu   :  {:6.4f}\n seg loss    :  {:6.4f}\n\nLevel 1:\n miou v1     : {:5.2f}\n miou v2     : {:5.2f}\n miou v3     : {:5.2f}\n seg accu    : {:6.4f}\n part diff   :  {:6.4f}\n struct accu :  {:6.4f}\n part accu   :  {:6.4f}\n seg loss    :  {:6.4f}\n'.format(
              i, miou_v1*100, miou_v2*100, miou_v3*100, avg_seg_accuracy, part_diff, structure_accuracy, avg_part_mask_accuracy, avg_seg_loss, miou_v1_1*100, miou_v2_1*100, miou_v3_1*100, avg_seg_accuracy_1, part_diff_1, structure_accuracy_1, avg_part_mask_accuracy_1, avg_seg_loss_1)
          ...
          if (i % FLAGS.disp_every_n_steps == 0):
            for it in range(test_iter_visual):
              [
                  ...
                  visual_point_gt_part_index_value,
                  visual_point_predict_part_index_value,
                  visual_point_predict_prob_value,
              ...
              n_point = visual_point_gt_part_index_value.size
              if n_point > 30000:
                select_index = np.linspace(0, n_point-1, num=30000, dtype=np.int32)
                visual_node_position_value = visual_node_position_value[select_index]
                visual_point_gt_part_index_value = visual_point_gt_part_index_value[select_index]
                visual_point_predict_part_index_value = visual_point_predict_part_index_value[select_index]
                visual_point_predict_prob_value = visual_point_predict_prob_value[select_index]
                visual_point_gt_part_index_1_value = visual_point_gt_part_index_1_value[select_index]
                visual_point_predict_part_index_1_value = visual_point_predict_part_index_1_value[select_index]
                visual_point_predict_prob_1_value = visual_point_predict_prob_1_value[select_index]

              pc_filename = os.path.join(obj_dir, 'pc_{:06d}_{:04d}.obj'.format(i, it))
              vis_pointcloud.save_points(visual_node_position_value,
                  visual_point_gt_part_index_value.flatten(), pc_filename, depth=5)

              pc_filename = os.path.join(obj_dir, 'pc_1_{:06d}_{:04d}.obj'.format(i, it))
              vis_pointcloud.save_points(visual_node_position_value,
                  visual_point_predict_part_index_value.flatten(), pc_filename, depth=5)

              visual_point_error_index = visual_point_predict_part_index_value.flatten()!=visual_point_gt_part_index_value.flatten()
              if FLAGS.delete_0:
                visual_point_error_index *= visual_point_gt_part_index_value.flatten() != 0
              pc_filename = os.path.join(obj_dir, 'pc_2_{:06d}_{:04d}.obj'.format(i, it))
              vis_pointcloud.save_points(visual_node_position_value, visual_point_error_index*4,
                  pc_filename, depth=5, squantial_color=True)

              visual_point_predict_prob_value = np.reshape(visual_point_predict_prob_value, [-1, n_part])
              point_predict_prob_entropy = entropy(visual_point_predict_prob_value, base=2, axis=1)
              point_predict_prob_entropy = (point_predict_prob_entropy/np.log2(n_part)*16).astype(np.int32)
              pc_filename = os.path.join(obj_dir, 'pc_3_{:06d}_{:04d}.obj'.format(i, it))
              vis_pointcloud.save_points(visual_node_position_value,
                  point_predict_prob_entropy.flatten(), pc_filename, depth=5, squantial_color=True)

              pc_filename = os.path.join(obj_dir, 'pc_4_{:06d}_{:04d}.obj'.format(i, it))
              vis_pointcloud.save_points(visual_node_position_value,
                  visual_point_gt_part_index_1_value.flatten(), pc_filename, depth=5)

              pc_filename = os.path.join(obj_dir, 'pc_5_{:06d}_{:04d}.obj'.format(i, it))
              vis_pointcloud.save_points(visual_node_position_value,
                  visual_point_predict_part_index_1_value.flatten(), pc_filename, depth=5)

              pc_filename = os.path.join(obj_dir, 'pc_6_{:06d}_{:04d}.obj'.format(i, it))
              vis_pointcloud.save_points(visual_node_position_value,
                  (visual_point_predict_part_index_1_value.flatten()!=visual_point_gt_part_index_1_value.flatten())*4,
                  pc_filename, depth=5, squantial_color=True)

              visual_point_predict_prob_1_value = np.reshape(visual_point_predict_prob_1_value, [-1, n_part_1])
              point_predict_prob_entropy_1 = entropy(visual_point_predict_prob_1_value, base=2, axis=1)
              point_predict_prob_entropy_1 = (point_predict_prob_entropy_1/np.log2(n_part)*16).astype(np.int32)
              pc_filename = os.path.join(obj_dir, 'pc_7_{:06d}_{:04d}.obj'.format(i, it))
              vis_pointcloud.save_points(visual_node_position_value,
                  point_predict_prob_entropy_1.flatten(), pc_filename, depth=5, squantial_color=True)
      ...
      all_part_gt_mask_1 = np.empty([test_iter, n_part_1], dtype=int)
      all_part_predict_mask_1 = np.empty([test_iter, n_part_1], dtype=int)
      all_point_predict_prob_value = np.empty([test_iter*n_test_point, n_part], dtype=np.float32)
      all_node_position = np.empty([test_iter*n_test_point, 3], dtype=np.float32)
      all_seg_loss = np.empty([test_iter], dtype=np.float32)
      all_seg_accuracy = np.empty([test_iter], dtype=np.float32)
      n_point_count = 0
      all_point_shape_index = np.empty([test_iter*n_test_point], dtype=int)
      for it in tqdm(range(test_iter)):
        ...
        all_part_gt_mask_1[it] = part_gt_mask_1_value
        all_part_predict_mask_1[it] = part_predict_mask_1_value
        all_point_predict_prob_value[n_point_count:n_point_count+n_shape_point, :] = point_predict_prob_value
        all_node_position[n_point_count:n_point_count+n_shape_point, :] = node_position_value[:, :3]
        all_seg_loss[it] = seg_loss_value
        all_seg_accuracy[it] = seg_accuracy_value
        all_point_shape_index[n_point_count:n_point_count+n_shape_point] = it
        n_point_count += n_shape_point

        if FLAGS.test_visual and it < test_iter_visual and it > 23:

          n_point = point_gt_part_index_value.size
          if n_point > 50000:
            select_index = np.linspace(0, n_point-1, num=50000, dtype=np.int32)
            node_position_value = node_position_value[select_index]
            point_gt_part_index_value = point_gt_part_index_value[select_index]
            point_predict_part_index_value = point_predict_part_index_value[select_index]
            point_predict_prob_value = point_predict_prob_value[select_index]

          pc_filename = os.path.join(obj_dir, 'pc_{:06d}_{:04d}.obj'.format(0, it))
          vis_pointcloud.save_points(node_position_value,
              point_gt_part_index_value.flatten(), pc_filename, depth=5)

          pc_filename = os.path.join(obj_dir, 'pc_1_{:06d}_{:04d}.obj'.format(0, it))
          vis_pointcloud.save_points(node_position_value,
              point_predict_part_index_value.flatten(), pc_filename, depth=5)

          point_error_index = point_predict_part_index_value.flatten()!=point_gt_part_index_value.flatten()
          if FLAGS.delete_0:
            point_error_index *= point_gt_part_index_value.flatten() != 0
          pc_filename = os.path.join(obj_dir, 'pc_2_{:06d}_{:04d}.obj'.format(0, it))
          vis_pointcloud.save_points(node_position_value, point_error_index*4,
              pc_filename, depth=5, squantial_color=True)
      ...
      all_point_gt_part_index_1 = all_point_gt_part_index_1[:n_point_count]
      all_point_predict_part_index_1 = all_point_predict_part_index_1[:n_point_count]
      all_point_predict_prob_value = all_point_predict_prob_value[:n_point_count]
      ...
      # np.save(os.path.join(FLAGS.logdir, 'all_point_gt_part_index_{}.npy'.format(test_iter)), all_point_gt_part_index)
      # np.save(os.path.join(FLAGS.logdir, 'all_point_predict_part_index_{}.npy'.format(test_iter)), all_point_predict_part_index)
      # np.save(os.path.join(FLAGS.logdir, 'all_point_shape_index_{}.npy'.format(test_iter)), all_point_shape_index)

      result_string = 'Iteration: {}\nLevel 0:\n miou v1     : {:5.2f}\n miou v2     : {:5.2f}\n miou v3     : {:5.2f}\n seg accu    : {:6.4f}\n part diff   :  {:6.4f}\n struct accu :  {:6.4f}\n part accu   :  {:6.4f}\n seg loss    :  {:6.4f}\n\nLevel 1:\n miou v1     : {:5.2f}\n miou v2     : {:5.2f}\n miou v3     : {:5.2f}\n seg accu    : {:6.4f}\n part diff   :  {:6.4f}\n struct accu :  {:6.4f}\n part accu   :  {:6.4f}\n seg loss    :  {:6.4f}\n'.format(start_iters - 1,
          miou_v1*100, miou_v2*100, miou_v3*100, avg_seg_accuracy, part_diff, structure_accuracy, avg_part_mask_accuracy, avg_seg_loss, miou_v1_1*100, miou_v2_1*100, miou_v3_1*100, avg_seg_accuracy_1, part_diff_1, structure_accuracy_1, avg_part_mask_accuracy_1, avg_seg_loss_1)
      print(result_string); sys.stdout.flush()
