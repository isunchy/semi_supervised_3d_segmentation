...
def octree_conv1x1_bn_lrelu(inputs, nout, training, alpha=0.1):
  conv = octree_conv1x1_bn(inputs, nout, training)
  return tf.nn.leaky_relu(conv, alpha=alpha)
...
def octree_upsample(data, octree, depth, channel, training, nempty=False):
...
def octree_conv_bn_relu(data, octree, depth, channel, training, kernel_size=[3],
  ...
  with tf.variable_scope('conv_bn_relu'):
    conv_bn = octree_conv_bn(data, octree, depth, channel, training, kernel_size,
                             stride, fast_mode, nempty)
    rl = tf.nn.relu(conv_bn)
  return rl


def octree_conv_bn_leakyrelu(data, octree, depth, channel, training, nempty=False):
  cb = octree_conv_bn(data, octree, depth, channel, training, nempty)
...
def octree_resblock(data, octree, depth, num_out, stride, training, bottleneck=4, nempty=False):
  ...
  with tf.variable_scope("3x3x3"):
    block2 = octree_conv_bn_relu(block1, octree, depth, channelb, training, nempty=nempty)
...
def octree_resblock2(data, octree, depth, num_out, training, nempty=False):
  num_in = int(data.shape[1])
  with tf.variable_scope("conv_1"):
    conv = octree_conv_bn_relu(data, octree, depth,  num_out/4, training, nempty=nempty)
  with tf.variable_scope("conv_2"):
    conv = octree_conv_bn(conv, octree, depth, num_out, training, nempty=nempty)
...
def octree_resblock3(data, octree, depth, num_out, training, nempty=False):
  num_in = int(data.shape[1])
  with tf.variable_scope("conv_1"):
    conv = octree_conv_bn_relu(data, octree, depth,  num_out, training, nempty=nempty)
  with tf.variable_scope("conv_2"):
    conv = octree_conv_bn(conv, octree, depth, num_out, training, nempty=nempty)
